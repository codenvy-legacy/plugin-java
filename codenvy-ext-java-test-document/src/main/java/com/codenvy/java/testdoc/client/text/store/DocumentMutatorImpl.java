...
import com.codenvy.java.testdoc.client.text.store.TextChange.Type;
import com.codenvy.java.testdoc.client.text.store.anchor.Anchor;
import com.codenvy.java.testdoc.client.text.store.anchor.AnchorManager;
...
class DocumentMutatorImpl implements TextStoreMutator {
    ...
    private void beginHighLevelModification(Type type, Line line, int lineNumber, int column, String text) {
     ...
     *
     * @param previousLine
     *         the line after which {@code line} will be inserted
     */
    ...
    private void deleteTextImpl(final Line firstLine, final int firstLineNumber, final int firstLineColumn,
                                final String deletedText) {
        Assert.isNotNull(firstLineColumn <= LineUtils.getLastCursorColumn(firstLine), "The column is out-of-bounds");
    ...
    private void removeLineImpl(Line line) {
       ...
       */
        ...
        if (previousLine != null) {
            ...
            assert line == document.getFirstLine() : "Line does not have a previous line, but line is not first line in document";
        ...
        if (nextLine != null) {
            ...
            assert line == document.getLastLine() : "Line does not have a next line, but line is not last line in document";
