...
import com.codenvy.java.testdoc.client.text.store.anchor.InsertionPlacementStrategy.Placement;
...
public class AnchorManager {
   ...
   */
     ...
     * @param line
     *         the line the anchor should attach to
     * @param lineNumber
     *         the line number, or {@link #IGNORE_LINE_NUMBER} if this
     *         anchor is not interested in its line number
     * @param column
     *         the column, or {@link #IGNORE_COLUMN} if this anchor is not
     ...
     *
     * @param anchor
     *         the @{link Anchor} anchor to start the search from
     * @param type
     *         the @{link AnchorType} of the anchor, or null to capture any type.
     * @param next
     *         true if the search is forwards, false for backwards
    ...
    /**
     * @param lastLineFirstUntouchedColumn
     *         the left-most column that was not part
    ...
    private void updateAnchorPositionWithoutDispatch(final Anchor anchor, Line line, int lineNumber,
        ...
        Assert.isNotNull(anchor.isAttached(), "Cannot move detached anchor");
     ...
     * {@code shiftAmount}.
     *
     * @param consultPlacementStrategy
     *         true to consult and obey the placement
