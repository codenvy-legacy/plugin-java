...
//	}
    ...
    private synchronized void computeAllRootPaths(IType type) {
        ...
        boolean containsJavaSource = !pkgFragmentRootPath
                .equals(this.sourcePath); // used to optimize zip file reading only if source path and root path are equals, otherwise
...
//											.validatePackageName(firstLevelPackageName, sourceLevel, complianceLevel);
                        ...
                        } else if (!containsJavaSource && org.eclipse.che.ide.ext.java.server.internal.core.util.Util.isJavaLikeFileName(
                                entryName)) {
...
//						manager = JavaModelManager.getJavaModelManager();
            ...
            try {
                ...
                for (Enumeration entries = zip.entries(); entries.hasMoreElements(); ) {
                    ZipEntry entry = (ZipEntry)entries.nextElement();
                    String entryName;
                    if (!entry.isDirectory() && Util.isJavaLikeFileName(
...
//			manager.flushZipFiles(this); // clean up cached zip files.
        ...
        if (VERBOSE) {
            System.out.println(
                    "spent " + (System.currentTimeMillis() - time) + "ms for " + type.getElementName()); //$NON-NLS-1$ //$NON-NLS-2$
        }
        return source;
    }
...
//		}
    ...
    protected IJavaElement[] getUnqualifiedMethodHandle(IMethod method, boolean noDollar) {
        ...
        for (int i = 0; i < qualifiedParameterTypes.length; i++) {
            ...
            getUnqualifiedTypeSignature(qualifiedParameterTypes[i], 0/*start*/, qualifiedParameterTypes[i].length(), unqualifiedTypeSig,
                                        noDollar);
    ...
    private int getUnqualifiedTypeSignature(String qualifiedTypeSig, int start, int length, StringBuffer unqualifiedTypeSig,
                                            boolean noDollar) {
        ...
        try {
            ...
            if (bytes != null) {
                ...
                return org.eclipse.jdt.internal.compiler.util.Util
                        .bytesToChar(bytes, charSet == null ? (this.encoding == null ? this.defaultEncoding : this.encoding) : charSet);
            }
        } catch (IOException e) {
            // ignore
        }
