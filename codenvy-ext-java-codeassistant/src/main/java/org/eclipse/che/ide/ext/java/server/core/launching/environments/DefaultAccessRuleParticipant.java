 ...
 *******************************************************************************/
package org.eclipse.che.ide.ext.java.server.core.launching.environments;

import org.eclipse.che.ide.ext.java.server.core.launching.IVMInstallType;
import org.eclipse.che.ide.ext.java.server.core.launching.LibraryLocation;
		...
		if (allRules == null || allRules.length != libraries.length) {
			...
			if (packages.length > 0) {
				...
				for (int i = 0; i < packages.length; i++) {
					packageRules[i] = JavaCore.newAccessRule(new Path(packages[i].replace('.', IPath.SEPARATOR)), IAccessRule
							.K_ACCESSIBLE);
				}
				// add IGNORE_IF_BETTER flag in case another explicit entry allows access (see bug 228488)
				packageRules[packages.length] =
						JavaCore.newAccessRule(new Path("**/*"), IAccessRule.K_NON_ACCESSIBLE | IAccessRule.IGNORE_IF_BETTER);
						//$NON-NLS-1$
			} else {
