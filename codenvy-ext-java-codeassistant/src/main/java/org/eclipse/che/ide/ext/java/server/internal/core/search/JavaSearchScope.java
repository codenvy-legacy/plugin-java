...
//						}
	...
	private int indexOf(String fullPath) {
		...
		for (int i = 0, length = this.relativePaths.length; i < length; i++) {
			...
			String currentFullPath =
					currentRelativePath.length() == 0 ? currentContainerPath : (currentContainerPath + '/' + currentRelativePath);
	 ...
	 *         5. c:\temp\mylib.jar
	 * @param relativePath
	 *         the forward slash path relatively to the container, e.g.
	 *         1. x/y/Z.class
	 *         2. x/y
	 *         3. X.java
	 *         4. (empty)
...
//        return null;
	...
	private void rehash() {
		JavaSearchScope
				newScope = new JavaSearchScope(this.pathsCount * 2);        // double the number of expected elements
