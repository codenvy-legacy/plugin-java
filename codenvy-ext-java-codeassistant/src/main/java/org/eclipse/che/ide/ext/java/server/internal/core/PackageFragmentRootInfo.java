...
class PackageFragmentRootInfo extends OpenableElementInfo {
	 ...
	 */
	static Object[] computeFolderNonJavaResources(IPackageFragmentRoot root, IContainer folder, char[][] inclusionPatterns,
			...
			nextResource: for (int i = 0; i < length; i++) {
				...
				switch (member.getType()) {
					case IResource.FILE :
						...
						if (Util.isValidCompilationUnitName(fileName, sourceLevel, complianceLevel) && !Util
								.isExcluded(member, inclusionPatterns, exclusionPatterns))
