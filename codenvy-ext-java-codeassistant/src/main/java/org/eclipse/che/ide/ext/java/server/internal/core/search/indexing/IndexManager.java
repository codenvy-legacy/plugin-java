...
import org.eclipse.che.ide.ext.java.server.core.JavaCore;
...
import org.eclipse.che.ide.ext.java.server.core.search.SearchEngine;
import org.eclipse.che.ide.ext.java.server.core.search.SearchParticipant;
import org.eclipse.che.ide.ext.java.server.internal.core.ClasspathEntry;
import org.eclipse.che.ide.ext.java.server.internal.core.JavaModelManager;
...
import org.eclipse.che.ide.ext.java.server.internal.core.search.PatternSearchJob;
...
//        if (JavaCore.getPlugin() == null) return;
     ...
     * Warning: Does not check whether index is consistent (not being used)
     */
    public synchronized Index getIndex(IPath containerPath, IndexLocation indexLocation, boolean reuseExistingFile,
        ...
        if (index == null) {
            ...
            if (reuseExistingFile) {
                if (indexLocation.exists()) { // check before creating index so as to avoid creating a new empty index if file is missing
                    try {
                        ...
                        if (currentIndexState != REBUILDING_STATE &&
                            ...
                            if (VERBOSE)
                                Util.verbose("-> cannot reuse existing index: " + indexLocation + " path: " +
                ...
                if (currentIndexState == REUSE_STATE) {
                    ...
                    if (VERBOSE)
                        Util.verbose("-> cannot reuse given index: " + indexLocation + " path: " +
                                     containerPathString); //$NON-NLS-1$ //$NON-NLS-2$
            ...
            if (createIfMissing) {
                try {
                    if (VERBOSE)
                        Util.verbose(
                                "-> create empty index: " + indexLocation + " path: " + containerPathString); //$NON-NLS-1$ //$NON-NLS-2$
                    index = new Index(indexLocation, containerPathString, false /*do not reuse index file*/);
                    this.indexes.put(indexLocation, index);
                    return index;
                } catch (IOException e) {
                    if (VERBOSE)
                        Util.verbose("-> unable to create empty index: " + indexLocation + " path: " +
                                     containerPathString); //$NON-NLS-1$ //$NON-NLS-2$
...
//        return new File(getJavaPluginWorkingLocation().toOSString());
    ...
    private boolean hasJavaLikeNamesChanged() {
        char[][] currentNames = org.eclipse.che.ide.ext.java.server.internal.core.search.Util.getJavaLikeExtensions();
...
//        IProject project = javaProject.getProject();
    ...
    public void scheduleDocumentIndexing(final SearchDocument searchDocument, IPath container, final IndexLocation indexLocation,
    ...
    private void readParticipantsIndexNamesFile() {
        ...
        try {
            char[] participantIndexNames =
                    org.eclipse.jdt.internal.compiler.util.Util.getFileCharContent(this.participantIndexNamesFile, null);
