...
import org.eclipse.che.ide.ext.java.server.internal.core.search.matching.JavaSearchNameEnvironment;
...
//			JavaProject javaProject = (JavaProject) model.getJavaProject(project);
			...
			this.cud = this.basicParser
					.parse(this.compilationUnit, new CompilationResult(this.compilationUnit, 0, 0, this.options.maxProblemsPerUnit));

			// Use a non model name environment to avoid locks, monitors and such.
			INameEnvironment nameEnvironment = new JavaSearchNameEnvironment(javaProject, /*JavaModelManager.getJavaModelManager()
			...
			for (int i = 0, length = this.cud.functionalExpressionsCount; i < length; i++) {
				...
				if (expression instanceof LambdaExpression) {
					...
					if (lambdaExpression.binding != null && lambdaExpression.binding.isValidBinding()) {
						...
						SourceIndexer.this.addIndexEntry(IIndexConstants.METHOD_DECL, MethodPattern
								.createIndexKey(lambdaExpression.descriptor.selector, lambdaExpression.descriptor.parameters.length));
