...
import com.codenvy.ide.ext.java.jdt.internal.corext.fix.UnusedCodeCleanUp;
...
public class ReorgCorrectionsSubProcessor {
    ...
    public static void getWrongTypeNameProposals(InvocationContext context, IProblemLocation problem,
                                                 Collection<ICommandAccess> proposals) {
        //TODO
        System.out.println("ReorgCorrectionsSubProcessor.getWrongTypeNameProposals()");
        ...
        //            && !JavaConventions.validateCompilationUnitName(newCUName, sourceLevel, compliance).matches(IStatus.ERROR))
        //         {
        ...
        //            proposals.add(new ChangeCorrectionProposal(label, change, 6, JavaPluginImages
        //               .get(JavaPluginImages.IMG_CORRECTION_RENAME)));
        //         }
        //      }
    }

    public static void getWrongPackageDeclNameProposals(InvocationContext context, IProblemLocation problem,
                                                        Collection<ICommandAccess> proposals) throws CoreException {
        //TODO
        System.out.println("ReorgCorrectionsSubProcessor.getWrongPackageDeclNameProposals()");
        //      boolean isLinked = cu.getResource().isLinked();
        //
        ...
        //         {
        //            label =
        //               Messages.format(CorrectionMessages.ReorgCorrectionsSubProcessor_movecu_default_description,
        ...
        //            label =
        //               Messages.format(CorrectionMessages.ReorgCorrectionsSubProcessor_movecu_description, new Object[]{
        //                  BasicElementLabels.getFileName(cu), packageLabel});
        //         }
        ...
        //
        //         proposals.add(new ChangeCorrectionProposal(label, composite, 6, JavaPluginImages
        //            .get(JavaPluginImages.IMG_CORRECTION_MOVE)));
        //      }
    }

    public static void removeImportStatementProposals(InvocationContext context, IProblemLocation problem,
                                                      Collection<ICommandAccess> proposals) {
        IProposableFix fix =
                UnusedCodeFix.createRemoveUnusedImportFix(context.getASTRoot(), problem, context.getDocument());
        if (fix != null) {
            Images image = Images.correction_delete_import;
        ...
        String name = CorrectionMessages.INSTANCE.ReorgCorrectionsSubProcessor_organizeimports_description();
        ChangeCorrectionProposal proposal =
                new ChangeCorrectionProposal(name, null, 5, Images.correction_change) {
                    @Override
                    public void apply(Document document) {
                        ...
                        //                  action.run(cu);
                        //               }
                        //TODO call Organize import action
//               IDE.fireEvent(new OrganizeImportsEvent());
                    }
                };
        proposals.add(proposal);
    }
    ...
    //      {
    //         super(CorrectionMessages.ReorgCorrectionsSubProcessor_project_seup_fix_description, cu, -10, JavaPluginImages
    ...
    //         if (ClasspathFixSelectionDialog.openClasspathFixSelectionDialog(shell, getCompilationUnit().getJavaProject(),
    ...
    //                  AddImportsOperation op =
    //                     new AddImportsOperation(getCompilationUnit(), fOffset, fLength, query, false, false);
    ...
    //   }
    //
    ////   public static void addProjectSetupFixProposal(IInvocationContext context, IProblemLocation problem,
    ////      String missingType, Collection<ICommandAccess> proposals)
    ////   {
    ////      proposals.add(new ClasspathFixCorrectionProposal(context.getCompilationUnit(), problem.getOffset(), problem
    ////         .getLength(), missingType));
    ////   }
    //
    public static void importNotFoundProposals(InvocationContext context, IProblemLocation problem,
                                               Collection<ICommandAccess> proposals) throws CoreException {
        ...
        if (!importDeclaration.isOnDemand()) {
            ...
            int kind = SimilarElementsRequestor.REF_TYPES;
            //            JavaModelUtil.is50OrHigher(cu.getJavaProject()) ? SimilarElementsRequestor.REF_TYPES
            //               : SimilarElementsRequestor.CLASSES | SimilarElementsRequestor.INTERFACES;
            UnresolvedElementsSubProcessor.addNewTypeProposals(name, kind, 5, proposals);
        }
    ...
    //            }
    //         }
    //         PreferencesUtil.createPropertyDialogOn(JavaPlugin.getActiveWorkbenchShell(), fProject,
    //            BuildPathsPropertyPage.PROP_ID, null, data).open();
    //      }
    //
    //      /*
    ...
    //   private static final class ChangeToRequiredCompilerCompliance extends ChangeCorrectionProposal implements
    //      IWorkspaceRunnable
    //   {
    //
    ...
    //
    //      public ChangeToRequiredCompilerCompliance(String name, IJavaProject project, boolean changeOnWorkspace,
    //         String requiredVersion, int relevance)
    //      {
    ...
    //               CorrectionMessages.ReorgCorrectionsSubProcessor_required_compliance_changeworkspace_description,
    //               fRequiredVersion));
    //         }
    //         else
    //         {
    //            message.append(Messages.format(
    //               CorrectionMessages.ReorgCorrectionsSubProcessor_required_compliance_changeproject_description,
    //               fRequiredVersion));
    //         }
    //
    //         try
    ...
    //                  {
    //                     message
    //                        .append(CorrectionMessages.ReorgCorrectionsSubProcessor_50_compliance_changeProjectJREToDefault_description);
    //                  }
    //                  if (defaultVM == null || !isRequiredOrGreaterVMInstall(defaultVM))
    //                  {
    //                     message.append(Messages.format(
    ...
    //         {
    //            MessageDialog
    //               .openInformation(JavaPlugin.getActiveWorkbenchShell(), Messages.format(
    //                  CorrectionMessages.ReorgCorrectionsSubProcessor_no_required_jre_title, fRequiredVersion), Messages
    //                  .format(CorrectionMessages.ReorgCorrectionsSubProcessor_no_required_jre_message, fRequiredVersion));
    //         }
    //      }
    //   }
    //
    ...
    //    */
    //   public static void getNeedHigherComplianceProposals(IInvocationContext context, IProblemLocation problem,
    //      Collection<ICommandAccess> proposals, String requiredVersion)
    //   {
    //      IJavaProject project = context.getCompilationUnit().getJavaProject();
    //
    //      String label1 =
    //         Messages.format(CorrectionMessages.ReorgCorrectionsSubProcessor_change_project_compliance_description,
    //            requiredVersion);
    ...
    //    * @param proposals the resulting proposals
    //    */
    //   public static void getIncorrectBuildPathProposals(IInvocationContext context, IProblemLocation problem,
    //      Collection<ICommandAccess> proposals)
    //   {
    ...
    public static void getAccessRulesProposals(InvocationContext context, IProblemLocation problem,
                                               Collection<ICommandAccess> proposals) {
        System.out.println("ReorgCorrectionsSubProcessor.getAccessRulesProposals()");
        //TODO
        ...
        //            new OpenBuildPathCorrectionProposal(project, label, 5, referencedElement);
        //         proposals.add(proposal);
        //      }
    }
    //
    ...
    //            IStatus status =
    //               classpathContainerInitializer.getAccessRulesStatus(classpathEntry.getPath(), root.getJavaProject());
    //            return status.isOK();
    //         }
    //      }
    //      catch (JavaModelException e)
