...
public class JavaDocAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {
     ...
     */
    private void indentAfterNewLine(Document d, DocumentCommand c) {
        ...
        try {
            ...
            Region line = d.getLineInformationOfOffset(p);
            ...
            Region prefix = findPrefixRange(d, line);
...
//	}
     ...
     * @return an <code>Region</code> describing the range of the prefix on the given line
     * @throws BadLocationException
     *         if accessing the document fails
     */
    private Region findPrefixRange(Document document, Region line) throws BadLocationException {
        int lineOffset = line.getOffset();
        ...
        return new RegionImpl(lineOffset, indentEnd - lineOffset);
    }
...
//	 */
//	private String createJavaDocTags(Document document, DocumentCommand command, String indentation, String lineDelimiter,
...
//	private String createTypeTags(Document document, DocumentCommand command, String indentation, String lineDelimiter, IType type)
//		throws CoreException, BadLocationException
//	{
//		String[] typeParamNames= StubUtility.getTypeParameterNames(type.getTypeParameters());
//		String comment= CodeGeneration.getTypeComment(type.getCompilationUnit(), type.getTypeQualifiedName('.'), typeParamNames,
// lineDelimiter);
//		if (comment != null) {
...
//	}
//
//	private String createMethodTags(Document document, DocumentCommand command, String indentation, String lineDelimiter, IMethod method)
//		throws CoreException, BadLocationException
//	{
//		Region partition= TextUtilities.getPartition(document, fPartitioning, command.offset, false);
...
//	 * @throws BadLocationException if accessing the document fails
//	 */
//	private boolean isFirstComment(Document document, DocumentCommand command, IMember member,
// boolean ignoreNonJavadoc) throws BadLocationException, JavaModelException {
//		Region partition= TextUtilities.getPartition(document, fPartitioning, command.offset, false);
...
//	}
     ...
     *         the command
     */
    private void indentAfterCommentEnd(Document d, DocumentCommand c) {
     ...
     */
    private boolean isNewComment(Document document, int commandOffset) {
        ...
        try {
            ...
            Region line = document.getLineInformation(lineIndex);
            TypedRegion partition = TextUtilities.getPartition(document, fPartitioning, commandOffset, false);
