...
public class JavaAutoEditStrategy extends DefaultIndentLineAutoEditStrategy {
     ...
     * @param max
     *         the max position
     * @return an insert position relative to the line start if <code>line</code> contains a parenthesized expression that can be followed
     *         by a block, -1 otherwise
     */
    private static int computeAnonymousPosition(Document document, int offset, String partitioning, int max) {
     ...
     * @return the position of a closing parenthesis left to <code>position</code> separated only by whitespace, or <code>position</code>
     *         if
     *         no parenthesis can be found
     */
     ...
     *         otherwise
     */
    private static boolean looksLikeAnonymousClassDef(Document document, String partitioning, JavaHeuristicScanner scanner, int position) {
     ...
     * @param partitioning
     *         the document partitioning
     * @return <code>true</code> if <code>position</code> is in the default partition of <code>document</code>, <code>false</code>
     *         otherwise
     */
    ...
    private static void installJavaStuff(Document document) {
        ...
        document.setDocumentPartitioner(JavaPartitions.JAVA_PARTITIONING, partitioner);
    }
    ...
    private static void removeJavaStuff(Document document) {
        document.setDocumentPartitioner(JavaPartitions.JAVA_PARTITIONING, null);
    }
    ...
    private static String getCurrentIndent(Document document, int line) throws BadLocationException {
        ...
        if (to > from && to < endOffset - 1 && document.get(to - 1, 2).equals(" *")) { //$NON-NLS-1$
            String type = TextUtilities.getContentType(document, JavaPartitions.JAVA_PARTITIONING, to, true);
            if (type.equals(JavaPartitions.JAVA_DOC) || type.equals(JavaPartitions.JAVA_MULTI_LINE_COMMENT))
                to--;
        }
...
//    }
    ...
    private static CompilationUnitInfo getCompilationUnitForMethod(Document document, int offset) {
        try {
            JavaHeuristicScanner scanner = new JavaHeuristicScanner(document);

            Region sourceRange = scanner.findSurroundingBlock(offset);
            if (sourceRange == null)
                return null;
            ...
            Log.error(JavaAutoEditStrategy.class, e);
        }
     ...
     */
    private static int getBlockBalance(Document document, int offset, String partitioning) {
        if (offset < 1)
            return -1;
    ...
    private static Region createRegion(ASTNode node, int delta) {
        return node == null ? null : new RegionImpl(node.getStartPosition() + delta, node.getLength());
    }

    private static Region getToken(Document document, Region scanRegion, int tokenId) {
        ...
        try {
            ...
            return new RegionImpl(tokenOffset + scanRegion.getOffset(), tokenLength);
    ...
    private int getBracketCount(Document d, int startOffset, int endOffset, boolean ignoreCloseBrackets) throws BadLocationException {
    ...
    private int getCommentEnd(Document d, int offset, int endOffset) throws BadLocationException {
    ...
    private String getIndentOfLine(Document d, int line) throws BadLocationException {
        if (line > -1) {
    ...
    private int getStringEnd(Document d, int offset, int endOffset, char ch) throws BadLocationException {
        while (offset < endOffset) {
            char curr = d.getChar(offset);
            offset++;
            if (curr == '\\') {
    ...
    private void smartIndentAfterClosingBracket(Document d, DocumentCommand c) {
    ...
    private void smartIndentAfterOpeningBracket(Document d, DocumentCommand c) {
        if (c.offset < 1 || d.getLength() == 0)
            return;
    ...
    private void smartIndentAfterNewLine(Document d, DocumentCommand c) {
        ...
        try {
            ...
            Region reg = d.getLineInformation(line);
            ...
            TypedRegion region = TextUtilities.getPartition(d, fPartitioning, start, true);
            if (JavaPartitions.JAVA_DOC.equals(region.getType()))
...
//    }
    ...
    private void smartPaste(Document document, DocumentCommand command) {
        ...
        try {
            ...
            Region refLine = document.getLineInformationOfOffset(refOffset);
...
//            DocumentRewriteSession session= temp.startRewriteSession(DocumentRewriteSessionType.STRICTLY_SEQUENTIAL);
            ...
            for (int l = first; l < lines; l++) { // we don't change the number of lines while adding indents
                ...
                Region r = temp.getLineInformation(l);
...
//            temp.stopRewriteSession(session);
     ...
     * @param difference
     *         a string buffer - if the return value is positive, it will be cleared and set to the substring of <code>current</code> of
     *         that length
     * @param tabLength
     *         the length of a tab
    ...
    private void addIndent(Document document, int line, CharSequence indent, int tabLength) throws BadLocationException {
        ...
        if (newInsert > insert) {
            ...
            if (whitespaceCount != 0 && whitespaceCount >= CodeFormatterUtil.getIndentWidth())
                insert = newInsert;
        }
    ...
    private int getPeerPosition(Document document, DocumentCommand command) {
    ...
    private void smartIndentOnKeypress(Document document, DocumentCommand command) {
    ...
    private void smartIndentUponE(Document d, DocumentCommand c) {
    ...
    /*
     * @see org.eclipse.jface.text.IAutoIndentStrategy#customizeDocumentCommand(org.eclipse.jface.text.Document,
...
//        IPreferenceStore preferenceStore = getPreferenceStore();
        ...
        fIsSmartTab = true; //preferenceStore.getBoolean(PreferenceConstants.EDITOR_SMART_TAB);
        fIsSmartIndentAfterNewline = true;//preferenceStore.getBoolean(PreferenceConstants.EDITOR_SMART_INDENT_AFTER_NEWLINE);
