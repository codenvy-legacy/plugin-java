...
import com.codenvy.ide.api.text.TextSelection;
import com.codenvy.ide.api.text.TextSelectionImpl;
 ...
 */
public class SmartSemicolonAutoEditStrategy implements AutoEditStrategy {
    ...
    /*
     * @see org.eclipse.jface.text.IAutoEditStrategy#customizeDocumentCommand(org.eclipse.jface.text.Document,
...
//            return;
        ...
        TextSelection line;
        try {
            Region l = document.getLineInformationOfOffset(pos);
            line = new TextSelectionImpl(document, l.getOffset(), l.getLength());
...
//						new RegionImpl(command.offset + command.text.length(), 0),
...
//			}
        ...
        command.length = 0;
        command.caretOffset = position + insertion.length();
...
//		}
     ...
     */
    private boolean isMultilineSelection(Document document, DocumentCommand command) {
        try {
     ...
     */
    private String adjustSpacing(Document doc, int position, char character) {
     ...
     */
    private boolean alreadyPresent(Document document, char ch, int position) {
     ...
     * @param offset
     *         the position of the caret in the line when <code>character</code> was typed
     * @param character
     ...
     */
    protected static int computeCharacterPosition(Document document, TextSelection line, int offset, char character, String partitioning) {
     ...
     * @param partitioning
     *         the document partitioning
     * @return an insert position  relative to the line start if <code>line</code> looks like being an array initialization at
     *         <code>offset</code>, -1 otherwise
     */
    private static int computeArrayInitializationPos(Document document, TextSelection line, int offset, String partitioning) {
     ...
     * @param offset
     *         the offset of the caret position, relative to the line start.
     * @return an insert position  relative to the line start if <code>line</code> contains one of the above keywords at or before
     *         <code>offset</code>, -1 otherwise
     */
    private static int computeAfterTryDoElse(Document doc, TextSelection line, int offset) {
     ...
     * @param partitioning
     *         the document partitioning
     * @return an insert position relative to the line start if <code>line</code> contains a parenthesized expression that can be
     * followed by a
     *         block, -1 otherwise
     */
    private static int computeAfterParenthesis(Document document, TextSelection line, int offset, String partitioning) {
     ...
     * @param partitioning
     *         the document partitioning
     * @return the position of a closing parenthesis left to <code>position</code> separated only by whitespace,
     * or <code>position</code> if no
     *         parenthesis can be found
     */
    private static int findClosingParenToLeft(Document document, int position, String partitioning) {
     ...
     * @param position
     *         the first character position in <code>document</code> to be considered
     * @return the position of a whitespace character greater or equal than <code>position</code> separated only by whitespace,
     * or -1 if none
     *         found
     */
    private static int firstWhitespaceToRight(Document document, int position) {
     ...
     * <code>scanTo</code>) that resides in a Java
     *         partition, or <code>-1</code> if none can be found
     */
    private static int firstNonWhitespaceBackward(Document document, int position, String partitioning, int bound) {
        ...
        try {
            while (position > bound) {
                char ch = document.getChar(position);
                if (!CharOperation.isWhitespace(ch) && isDefaultPartition(document, position, partitioning))
                    return position;
                position--;
            }
        } catch (BadLocationException e) {
     ...
     * @param bound
     *         the first position in <code>document</code> to not consider any more, with <code>bound</code> &gt; <code>position</code>
     * @return the smallest position of one element in <code>chars</code> in [<code>position</code>, <code>scanTo</code>) that resides in a
     *         Java partition, or <code>-1</code> if none can be found
     */
    private static int firstNonWhitespaceForward(Document document, int position, String partitioning, int bound) {
        Assert.isTrue(position >= 0);
     ...
     * @return the highest position of one element in <code>chars</code> in (<code>bound</code>, <code>position</code>] that resides in a
     * Java
     *         partition, or <code>-1</code> if none can be found
     */
    private static int scanBackward(Document document, int position, String partitioning, int bound, char[] chars) {
        Assert.isTrue(bound >= -1);
        Assert.isTrue(position < document.getLength());
...
//	 * @param bound the first position in <code>document</code> to not consider any more, with <code>scanTo</code> &gt; <code>position
// </code>
//	 * @param chars an array of <code>char</code> to search for
//	 * @return the highest position of one element in <code>chars</code> in [<code>position</code>,
// <code>scanTo</code>) that resides in a Java partition, or <code>-1</code> if none can be found
//	 */
//	private static int scanBackward(Document document, int position, int bound, char ch) {
...
//
     ...
     * <code>bound</code>) that resides in a Java
     *         partition, or <code>-1</code> if none can be found
     */
    private static int scanForward(Document document, int position, String partitioning, int bound, char[] chars) {
        Assert.isTrue(position >= 0);
        Assert.isTrue(bound <= document.getLength());
     ...
     *         the first position in <code>document</code> to not consider any more, with <code>scanTo</code> &gt; <code>position</code>
     * @param ch
     *         a <code>char</code> to search for
     * @return the lowest position of one element in <code>chars</code> in [<code>position</code>,
     * <code>bound</code>) that resides in a Java
     *         partition, or <code>-1</code> if none can be found
     */
    private static int scanForward(Document document, int position, String partitioning, int bound, char ch) {
     ...
     */
    private static boolean isNewMatch(Document document, int offset, int length, String partitioning) {
     ...
     * @param partitioning
     *         the document partitioning
     * @return <code>true</code> if the content of <code>document</code> looks like an anonymous class definition, <code>false</code>
     *         otherwise
     */
    private static boolean looksLikeAnonymousClassDef(Document document, int position, String partitioning) {
     ...
     */
    private static boolean isDefaultPartition(Document document, int position, String partitioning) {
        Assert.isTrue(position >= 0);
        Assert.isTrue(position <= document.getLength());

        try {
            // don't use getPartition2 since we're interested in the scanned character's partition
            TypedRegion region = TextUtilities.getPartition(document, partitioning, position, false);
            return region.getType().equals(Document.DEFAULT_CONTENT_TYPE);
     ...
     */
    private static int findOpeningParenMatch(Document document, int position, String partitioning) {
        final char CLOSING_PAREN = ')';
     ...
     * @param partitioning
     *         the document partitioning
     * @return <code>true</code> if <code>document</code> contains any of the above keywords to the left of <code>position</code>,
     *         <code>false</code> otherwise
     */
    private static boolean looksLikeIfWhileForCatch(Document document, int position, String partitioning) {
     ...
     * @return <code>true</code> if  <code>document</code> contains <code>like</code> such that it ends at <code>position</code>,
     *         <code>false</code> otherwise
     */
    private static boolean looksLike(Document document, int position, String like) {
     ...
     */
    private static boolean looksLikeMethodDecl(Document document, int position, String partitioning) {
     ...
     */
    private static int eatBrackets(Document document, int position, String partitioning) {
     ...
     */
    private static int eatIdentToLeft(Document document, int position, String partitioning) {
        if (position < 0)
            return -1;
        Assert.isTrue(position < document.getLength());
        ...
        try {
            while (p >= 0) {
                ...
                // length must be > 0
                if (CharOperation.isWhitespace(ch) && p != position)
                    return p + 1;
                else
                    return -1;
     ...
     */
    private static int nextPartitionOrLineEnd(Document document, TextSelection line, int offset, String partitioning) {
        ...
        try {
            TypedRegion partition = TextUtilities.getPartition(document, partitioning, nextPartitionPos, true);
     ...
     */
    private static int getValidPositionForPartition(Document doc, TypedRegion partition, int maxOffset) {
        final int INVALID = -1;

        if (JavaPartitions.JAVA_DOC.equals(partition.getType()))
            return INVALID;
        if (JavaPartitions.JAVA_MULTI_LINE_COMMENT.equals(partition.getType()))
            return INVALID;
        if (JavaPartitions.JAVA_SINGLE_LINE_COMMENT.equals(partition.getType()))
            return INVALID;

        int endOffset = Math.min(maxOffset, partition.getOffset() + partition.getLength());

        if (JavaPartitions.JAVA_CHARACTER.equals(partition.getType()))
            return endOffset;
        if (JavaPartitions.JAVA_STRING.equals(partition.getType()))
            return endOffset;
    ...
    private static int startOfWhitespaceBeforeOffset(String text, int offset) {
        ...
        for (; i >= 1; i--) {
            if (!CharOperation.isWhitespace(text.charAt(i - 1)))
                break;
        }
        return i;
    }
