...
/**
 * IMPORTANT NOTE: Internal Scanner implementation. It is mirrored in org.eclipse.jdt.core.compiler public package where it is
 * API. The mirror implementation is using the backward compatible ITerminalSymbols constant definitions (stable with 2.0),
 * whereas the internal implementation uses TerminalTokens which constant values reflect the latest parser generation state.
 */
public class Scanner implements TerminalTokens {
    ...
    */
                ...
                for (int itag = 0; itag < this.taskTags.length; itag++) {
                    ...
                    if (this.foundTaskTags == null) {
                        ...
                        System.arraycopy(this.foundTaskTags, 0, this.foundTaskTags = new char[this.foundTaskCount * 2][], 0,
                                         this.foundTaskCount);
                        System.arraycopy(this.foundTaskMessages, 0, this.foundTaskMessages =
                                new char[this.foundTaskCount * 2][], 0, this.foundTaskCount);
                        ...
                        System.arraycopy(this.foundTaskPositions, 0, this.foundTaskPositions =
                                new int[this.foundTaskCount * 2][], 0, this.foundTaskCount);
                    }

                    priority =
                            this.taskPriorities != null && itag < this.taskPriorities.length ? this.taskPriorities[itag] : null;
    ...
    public char[] getCurrentTokenSource() {
        ...
        if (this.withoutUnicodePtr != 0)
            ...
            int length;
            System.arraycopy(this.source, this.startPosition, result =
                    new char[length = this.currentPosition - this.startPosition], 0, length);
        }
        return result;
    }
    ...
    public char[] getCurrentTokenSourceString() {
        ...
        if (this.withoutUnicodePtr != 0)
            ...
            int length;
            System.arraycopy(this.source, this.startPosition + 1, result =
                    new char[length = this.currentPosition - this.startPosition - 2], 0, length);
        }
        return result;
    }
    ...
    */
    ...
    /*
    * External API in JavaConventions. This is used to optimize the case where the scanner is used to scan a single identifier. In
    * this case, the AIOOBE is slower to handle than a bound check
    */
        ...
        while (true) { // loop for jumping over comments
            ...
            do {
                ...
                if (this.currentPosition < this.eofPosition) {
                    this.currentCharacter = this.source[this.currentPosition++];
                    checkIfUnicode =
                            this.currentPosition < this.eofPosition && this.currentCharacter == '\\'
                            && this.source[this.currentPosition] == 'u';
        ...
        try {
            while (true) { // loop for jumping over comments
                ...
                switch (this.currentCharacter) {
                    case '@':
                  /*
                   * if (this.sourceLevel >= ClassFileConstants.JDK1_5) { return TokenNameAT; } else { return TokenNameERROR; }
                   */
                        return TokenNameAT;
                    case '(':
                           ...
                           /* We need to completely consume the line break */
                            ...
                            if (test > 0) { // traditional and javadoc comment
                                try { // get the next char
                                    ...
                                    if (this.tokenizeComments) {
                              /*
                               * if (isJavadoc) return TokenNameCOMMENT_JAVADOC; return TokenNameCOMMENT_BLOCK;
                               */
                                        return token;
                                    }
                                } catch (IndexOutOfBoundsException e) {
                    ...
                    case '\u001a':
                        if (atEnd())
                            return TokenNameEOF;
                        // the atEnd may not be <currentPosition == source.length> if source is only some part of a real (external)
                        // stream
    ...
    public void getNextUnicodeChar() throws InvalidInputException {
        ...
        if ((c1 = ScannerHelper.getNumericValue(this.source[this.currentPosition++])) > 15 || c1 < 0
            || (c2 = ScannerHelper.getNumericValue(this.source[this.currentPosition++])) > 15 || c2 < 0
            || (c3 = ScannerHelper.getNumericValue(this.source[this.currentPosition++])) > 15 || c3 < 0
            || (c4 = ScannerHelper.getNumericValue(this.source[this.currentPosition++])) > 15 || c4 < 0) {
            throw new InvalidInputException(INVALID_UNICODE_ESCAPE);
        }
    ...
    final char[] optimizedCurrentTokenSource6() {
        ...
        int hash = (((c0 = src[start]) << 12) + ((c2 = src[start + 2]) << 6) + (c4 = src[start + 4])) % TableSize;
        // int hash = (int)(((((long) c0) << 32) + (((long) c1) << 24) + (((long) c2) << 18) + (c3 << 12) + (c4 << 6) + c5) %
        // TableSize);
    ...
    private void parseTags() {
        ...
        if (this.withoutUnicodePtr != 0) {
            // 0 is used as a fast test flag so the real first char is in position 1
            System
                    .arraycopy(this.withoutUnicodeBuffer, 1, s = new char[this.withoutUnicodePtr], 0, this.withoutUnicodePtr);
    ...
    /**
     * Reposition the scanner on some portion of the original source. The given endPosition is the last valid position. Beyond this
     * position, the scanner will answer EOF tokens (<code>ITerminalSymbols.TokenNameEOF</code>).
     *
     * @param begin
...
// thus we use a direct access to the source (for regular cases).
            ...
            while (true) {
                ...
                if (c < ScannerHelper.MAX_OBVIOUS) {
                    if ((ScannerHelper.OBVIOUS_IDENT_CHAR_NATURES[c] & (ScannerHelper.C_UPPER_LETTER
                                                                        | ScannerHelper.C_LOWER_LETTER | ScannerHelper.C_IDENT_PART |
                        ...
                        this.currentPosition++;
                    } else if ((ScannerHelper.OBVIOUS_IDENT_CHAR_NATURES[c] & (ScannerHelper.C_SEPARATOR | ScannerHelper.C_JLS_SPACE)) !=
                               0) {
                        this.currentCharacter = c;
                        break identLoop;
                    } else {
            ...
            case 'e': // else extends
                switch (length) {
                    case 4:
                        ...
                        return TokenNameIdentifier;
                    case 7:
                        if ((data[++index] == 'x') && (data[++index] == 't') && (data[++index] == 'e')
                            && (data[++index] == 'n') && (data[++index] == 'd') && (data[++index] == 's'))
    ...
    /*
    * Should be used if a parse (usually a diet parse) has already been performed on the unit, so as to get the already computed
    * line end positions.
    */
