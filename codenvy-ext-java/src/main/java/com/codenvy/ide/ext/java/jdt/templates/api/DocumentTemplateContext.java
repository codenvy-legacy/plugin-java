 ...
 * its offset and length, or by a <code>Position</code> which may or may not be registered with the document.
 * <p>
...
public class DocumentTemplateContext extends TemplateContext {
    ...
    /**
     * The region of the document described by this context. We store a position since clients may specify the document region as
     * (updateable) Positions.
     */
    private final Position fPosition;

    /** The original offset of this context. Will only be updated by the setter method. */
    private int fOriginalOffset;

    /** The original length of this context. Will only be updated by the setter method. */
     ...
     */
    public DocumentTemplateContext(TemplateContextType type, Document document, int offset, int length) {
        this(type, document, new Position(offset, length));
    }

    /**
     * Creates a document template context. The supplied <code>Position</code> will be queried to compute the <code>getStart</code>
     * and <code>getEnd</code> methods, which will therefore answer updated position data if it is registered with the document.
     *
     * @param type
     *         the context type
     * @param document
     *         the document this context applies to
     * @param position
     *         the position describing the area of the document which forms the template context
     * @since 3.1
     */
    public DocumentTemplateContext(TemplateContextType type, Document document, Position position) {
        super(type);
