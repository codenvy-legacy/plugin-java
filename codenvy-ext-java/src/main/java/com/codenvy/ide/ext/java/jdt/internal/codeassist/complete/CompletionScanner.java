...
public class CompletionScanner extends Scanner {
    ...
    /*
     * Source positions of the completedIdentifier if inside actual identifier, end goes to the actual identifier end, in other
     * words, beyond cursor location
     */
    ...
    /*
     * Truncate the current identifier if it is containing the cursor location. Since completion is performed on an identifier
     * prefix.
     */
    public char[] getCurrentIdentifierSource() {

        if (this.completionIdentifier == null) {
            if (this.cursorLocation < this.startPosition &&
                this.currentPosition == this.startPosition) { // fake empty identifier got issued
        ...
        if (this.completionIdentifier == null) {
            if (this.cursorLocation + 1 >= this.startPosition && this.cursorLocation < this.currentPosition) {
                ...
                if (this.withoutUnicodePtr != 0) { // check unicode scenario
                    ...
                    System.arraycopy(this.source, this.startPosition + 1, (this.completionIdentifier = new char[length]), 0,
                                     length);
                }
                return this.completionIdentifier;
            }
            ...
            while (true) { // loop for jumping over comments
               ...
               /* completion requesting strictly inside blanks */
                    if ((whiteStart != this.currentPosition)
                        ...
                        && (this.completionIdentifier == null) && (whiteStart <= this.cursorLocation + 1)
               ...
               /* might be completing at eof (e.g. behind a dot) */
                ...
                switch (this.currentCharacter) {
                    ...
                    case '/': {
                        ...
                        /* We need to completely consume the line break */
                                if (this.currentCharacter == '\r' && this.eofPosition > this.currentPosition) {
                                    if (this.source[this.currentPosition] == '\n') {
                                        ...
                                        // -------------unicode traitement ------------
                                        int c1 = 0, c2 = 0, c3 = 0, c4 = 0;
                                        if ((c1 = ScannerHelper.getNumericValue(this.source[index++])) > 15 || c1 < 0
                                            || (c2 = ScannerHelper.getNumericValue(this.source[index++])) > 15 || c2 < 0
                                            || (c3 = ScannerHelper.getNumericValue(this.source[index++])) > 15 || c3 < 0
                                            || (c4 = ScannerHelper.getNumericValue(this.source[index++])) > 15 || c4 < 0) {
                        ...
                        if (test > 0) { // traditional and javadoc comment
                            try { // get the next char
                                ...
                                if (!isJavadoc && this.startPosition <= this.cursorLocation
                                    && this.cursorLocation < this.currentPosition - 1) {
                                    throw new InvalidCursorLocation(InvalidCursorLocation.NO_COMPLETION_INSIDE_COMMENT);
                                }
                                if (this.taskTags != null)
                    ...
                    case '\u001a':
                        if (atEnd())
                            return TokenNameEOF;
                        // the atEnd may not be <this.currentPosition == this.source.length> if source is only some part of a real
                        // (external) stream
    ...
    protected boolean isFirstTag() {
        return getNextChar('d') && getNextChar('e') && getNextChar('p') && getNextChar('r') && getNextChar('e')
               && getNextChar('c') && getNextChar('a') && getNextChar('t') && getNextChar('e') && getNextChar('d');
    }
