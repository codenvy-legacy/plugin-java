...
/**
 * Uses the {@link JavaHeuristicScanner} to
...
//            return fProject.getOption(key, true);
        ...
        private boolean prefUseTabs() {
            return false; //!JavaCore.SPACE.equals(getCoreFormatterOption(DefaultCodeFormatterConstants.FORMATTER_TAB_CHAR));
        }
        ...
        private int prefMethodBodyIndent() {
            if (DefaultCodeFormatterConstants.FALSE.equals(getCoreFormatterOption(
    ...
    public StringBuffer computeIndentation(int offset, boolean assumeOpeningBrace) {
        ...
        if (fAlign != JavaHeuristicScanner.NOT_FOUND) {
            try {
                ...
                Region line = fDocument.getLineInformationOfOffset(fAlign);
    ...
    private StringBuffer getLeadingWhitespace(int offset) {
        ...
        try {
            Region line = fDocument.getLineInformationOfOffset(offset);
            int lineOffset = line.getOffset();
     ...
     *         whether to convert consecutive runs of
     *         spaces to tabs
    ...
    private StringBuffer createIndent(int start, final int indent, final boolean convertSpaceRunsToTabs) {
        ...
        try {
            ...
            while (start < indent) {
                ...
                if (ch == '\t') {
                    ret.append(StringUtils.repeatString(" ", tabLen));
                    spaces = 0;
                } else if (convertTabs) {
                    spaces++;
//                    if (spaces == tabLen) {
...
//                    }
     ...
     *
     * @param offset
     *         the offset for which the check is done
                ...
                switch (fToken) {
                    ...
                    case Symbols.TokenEQUAL:
                        int stringStartingOffset =
                                fScanner.findNonWhitespaceForwardInAnyPartition(fPosition + 1, JavaHeuristicScanner.UNBOUND);
     ...
     * @param matchBrace
     *         whether the position of the matching brace should be returned instead of
     *         doing code analysis
     * @param matchParen
     *         whether the position of the matching parenthesis should be returned instead
     ...
     *         (either an earlier case statement or the switch block brace)
     * @param throwsClause
     *         whether a throws clause was found at <code>position</code>
     * @return the reference statement relative to which <code>position</code> should be indented,
     *         or {@link JavaHeuristicScanner#NOT_FOUND}
     * @since 3.7
     */
     ...
     * @param isInBlock
     *         whether the current position is inside a block, which limits the search scope to the next scope introducer
