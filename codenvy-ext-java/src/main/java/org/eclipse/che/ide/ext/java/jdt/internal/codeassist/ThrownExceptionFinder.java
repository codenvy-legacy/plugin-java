...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.ThrowStatement;
...
public class ThrownExceptionFinder extends ASTVisitor {
    ...
    /**
     * Finds the thrown exceptions minus the ones that are already caught in previous catch blocks. Exception is already caught
     * even if its super type is being caught. Also computes, separately, a list comprising of (a)those exceptions that have been
     * caught already and (b)those exceptions that are thrown by the method and whose super type has been caught already.
     *
     * @param tryStatement
     * @param scope
     */
     ...
     *
     * @return Returns an array of those exceptions that have been caught already in previous catch or multi-catch blocks of the
     *         same try block. (Exceptions caught in inner try-catches are obtained via
    ...
    /**
     * Returns all exceptions that are discouraged to use because (a) they are already caught in some inner try-catch, or (b) their
     * super exception has already been caught.
     *
        ...
        for (int i = 0; i < length; i++) {
            if (catchArguments[i].type instanceof UnionTypeReference) {
                ...
                for (int j = 0; j < unionTypeReference.typeReferences.length; j++) {
                    ...
                    if ((caughtException instanceof ReferenceBinding) &&
                        caughtException.isValidBinding()) { // might be null when its the completion node
