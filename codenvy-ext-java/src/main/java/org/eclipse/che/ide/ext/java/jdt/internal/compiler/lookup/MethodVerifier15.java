            ...
            if (originalInherited.declaringClass.isInterface()) {
                if ((concreteMethod.declaringClass == this.type.superclass && this.type.superclass.isParameterizedType() &&
                     !areMethodsCompatible(
                             concreteMethod, originalInherited))
            ...
            if (current == null && this.type.isPublic()) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    if (inheritedMethod.isPublic() && !inheritedMethod.declaringClass.isPublic())
                        if (!CharOperation.equals(inheritedMethod.declaringClass.qualifiedPackageName(),
                                                  CharOperation.NO_CHAR)) // https://bugs.eclipse.org/bugs/show_bug.cgi?id=343060
            ...
            for (int i = 0, length = inherited.length; i < length; i++) {
                ...
                if (index > 0) {
                    ...
                    while (--count > 0) {
                        ...
                        if (interfaceMethod != null && implementation != null
                        ...
                        if (first.declaringClass.isInterface() && match.declaringClass.isInterface()
    ...
    void verify() {
        ...
        for (int i = this.type.typeVariables.length; --i >= 0; ) {
            ...
            for (int j = itsInterfaces.length; --j >= 0; ) {
                superInterfaces[j] =
                        itsInterfaces[j].kind() == Binding.TYPE_PARAMETER ? (ReferenceBinding)itsInterfaces[j].erasure()
                                                                          : itsInterfaces[j];
            }
